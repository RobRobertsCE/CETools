...
Public Class VersionFileMonitor
...
#Region " Constants "
    ...
    Private Const PfsConnectFilePath As String = "C:\PfsCommon\PfsConnect.ini"
#End Region
...
#Region " Protected "
    Protected Overridable Sub BeginMonitorBranch()
        Try
            ...
            OnBranchVersionChanged(GetCurrentVersionFileVersion())
        Catch ex As Exception
            ExceptionHandler(ex)
        End Try
    End Sub
    ...
    Protected Overridable Sub HandleVersionChange()
        Try
            Dim newVersion As Version = GetCurrentVersionFileVersion()
    ...
    Protected Overridable Function GetCurrentPfsConnection() As String
        Dim testDatabaseName As String = String.Empty
        Dim testServerName As String = String.Empty
        Try
            Dim fileLines = File.ReadAllLines(PfsConnectFilePath)
            For Each line As String In fileLines
                If line.StartsWith("Catalog=") Then
                    testDatabaseName = line.Replace("Catalog=", "")
                ElseIf line.StartsWith("DataSource=") Then
                    testServerName = line.Replace("DataSource=", "")
                End If
            Next
        Catch ex As Exception
            ExceptionHandler(ex)
        End Try
        Return String.Format("{0}:{1}", testServerName, testDatabaseName)
    End Function

    Protected Overridable Function GetCurrentVersionFileVersion() As Version
...
#Region " Public "
    ...
    Public Function GetTestDatabase() As String
        Dim databaseName As String = String.Empty
        Try
            databaseName = GetCurrentPfsConnection()
        Catch ex As Exception
            ExceptionHandler(ex)
        End Try
        Return databaseName
    ...
    Public Function GetVersionFileVersion() As Version
        Dim currentVersion As Version = Nothing
        Try
            currentVersion = GetCurrentVersionFileVersion()
        Catch ex As Exception
            ExceptionHandler(ex)
        End Try
